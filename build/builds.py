...
class BuildList():
    def __init__(self, bot):
        if bot != None:
            # 配装指令
            @bot.command(regex=r'/hdzp|火电重炮',case_sensitive=False)
            async def huodianzhongpao(msg: Message):
                await reqFrontLogger(msg,bot)
                await msg.reply(Build().getBuild('huodianzhongpao'))

            @bot.command(regex=r'/hd|火电',case_sensitive=False)
            async def huodian(msg: Message):
                await reqFrontLogger(msg,bot)
                await msg.reply(Build().getBuild('huodian'))

            @bot.command(regex=r'/jgj|坚果狙',case_sensitive=False)
            async def jianguoju(msg: Message):
                await reqFrontLogger(msg,bot)
                await msg.reply(Build().getBuild('lieyanhabao'))
    ...
    def getList(self) -> CardMessage:
        c = Card(Module.Header('常用配装指南（配装表2.1）'), color='#FFCC00')
        ...
        c.append(Module.Section('全火输出(9)'))
        c.append(Module.Context('全部属性致力于提升武器伤害，不关注生存，追求极致的输出能力，但身板很脆，在有队友辅助的情况下是绝对的团队输出核心'))
        c.append(Module.Divider())
        c.append(Module.Section({"type":"kmarkdown","content":"雷神突袭(T0火爆)              快捷指令：`/tx` `突袭`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"烈焰哈堡坚果狙(T0暴力无脑)  快捷指令：`/jgj` `坚果狙`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"王子手枪盾(T1)                 快捷指令：`/wz` `王子`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"全火探子警戒(T1推荐)       快捷指令：`/tj` `探子警戒`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"谈判专注AR(T1高群伤)      快捷指令：`/tp` `谈判`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"打牌猎头狙(T1)                 快捷指令：`/dp` `打牌` `打牌狙`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"先锋猎头狙(T1)                 快捷指令：`/xf` `先锋` `先锋狙`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"金装全火AR(T2三天命)      快捷指令：`/qh` `全火` `金装全火`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"金装全火AR(T2无天命)      快捷指令：`/qh2` `全火2` `金装全火2`"}))
        # 火体生存
        c.append(Module.Divider())
        c.append(Module.Section('火体生存(4)'))
        c.append(Module.Context('兼顾进攻与防守，往往依靠高阶盾牌和叠加大量的额外装甲抵御敌方的攻击，同时能够打出可观的伤害，是吸引仇恨、保护队友、冲锋陷阵的首选'))
        c.append(Module.Divider())
        c.append(Module.Section({"type":"kmarkdown","content":"猎人追悼天蝎座(T1)    快捷指令：`/lr` `猎人` `猎人喷`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"火体芳心(T1)              快捷指令：`/fx` `芳心` `火体芳心`"}))
        ...
        # 火电
        c.append(Module.Divider())
        c.append(Module.Section('火电(4)'))
        c.append(Module.Context('拥有高技能分阶的同时，凭借电容突击的特点也拥有高武器伤害，即可使用武器输出，同时也可以根据战场需要灵活搭配技能做到多点输出或全能辅助'))
        c.append(Module.Section({"type":"kmarkdown","content":"忍者包火电突袭(7火6电)  快捷指令：`/hd` `火电`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"忍者包火电重炮(6火6电)  快捷指令：`/hdzp` `火电重炮`"}))
        # 电工塔防
        c.append(Module.Divider())
        ...
        c.append(Module.Divider())
        c.append(Module.Section({"type":"kmarkdown","content":"波皇塔防(必备)           快捷指令：`/tf` `塔防`"}))
        ...
        c.append(Module.Divider())
        c.append(Module.Section({"type":"kmarkdown","content":"超载未来奶妈(必备)     快捷指令：`/nm` `奶妈` `未来奶`"}))
        c.append(Module.Section({"type":"kmarkdown","content":"日蚀火法控制(必备)     快捷指令：`/rs` `日蚀` `日蚀控`"}))
